...
+ (BOOL)clearCache;//ijk自带缓存清除
+ (NSNumber *)cacheSize;//ijk自带缓存大小

// flag=true:启用自带缓存 flag=false:关闭自带缓存 （默认：关闭）
//+ (void)disableProxy:(BOOL)flag;

/// 启动VR播放 (必须在创建对象之前设置,并且设置效果会持续,正常播放请前提前关闭)
/// @param isStart (YES 开启  NO 关闭)
+ (void)startVRPlayView:(BOOL)isStart;

// 设置yes为不继续播放，设置no为正常逻辑。
- (void)networkIsChange:(BOOL)flag;
- (void)prepareToPlay;
- (void)play;
- (void)pause;
- (void)stop;
